...
function* getReadyUsersRequestWorker () {
    const newReadyUsers : IFBUserData[] = yield call(databaseService.getGameMembers);
    yield put(setGameMembers(newReadyUsers));
};

function* getVotedToStartWorker () {
    const newVotedToStart : IFBUserData[] = yield call(databaseService.getVotedToStartMembers);
...
export default function* gameSagas () {
    ...
    yield takeEvery(GameActions.GET_READY_USERS_REQUEST, getReadyUsersRequestWorker);
